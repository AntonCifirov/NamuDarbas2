...
Duom::Duom(string Var, string Pav, vector<double> Paz, double Egz) : mVar(Var), mPav(Pav), mPaz(Paz), mEgz(Egz) 
{
    galutinis();
    galutinis_m();
}
...
Duom Duom::eilutes_nuskaitymas(ifstream& kursiokai, int nd_sk)
{
    kursiokai >> mVar;
    kursiokai >> mPav;
    int p;
...
void Failo_nuskaitymas(string failo_pavadinimas, vector<Duom>& asmuo1) {
    ...
    for (int i = 0; i < stud_sk; i++)
    {
        asmuo1.push_back(asmuo.eilutes_nuskaitymas(kursiokai, nd_sk));
    }
